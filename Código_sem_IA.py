...
03- Filtrar Info > Permite o usuário escolher que tipo de informação ele deseja ver dentre as opções: CPF, Gênero, Religião, RG e Telefone (necessário escolher o diretório antes)
...
LEMBRE-SE DE SEMPRE SALVAR AS CONFIGURAÇÕES!!! 
    ...
    def graphic(self):
        messagebox.showinfo("DICA", "O gráfico que lista o caminho dos diretórios é separado em cores, onde: Azul - TOP 1, Amarelo - TOP 2, Verde - TOP 3!")
        ...
        # Preparamos o conjunto de dados para o total de informações encontradas no conjunto mais recente
        total_info_latest = latest_data['Informação encontrada'].value_counts()

        # Definindo o tamanho da figura para 1200x700 pixels
        ...
        ax1.set_title('Total de Informações Sensíveis Encontradas', fontsize=10, fontweight='bold')

        # Calculando a porcentagem de informações sensíveis para cada diretório
        total_directories_info = latest_data['Diretório'].value_counts()
        total_info = total_directories_info.sum()
        top_directories = total_directories_info.head(3)
        directory_percents = (top_directories / total_info) * 100
        other_percent = 100 - directory_percents.sum()

        # Concatenando os dados dos diretórios com o valor de "Outros"
        pie_data = pd.concat([directory_percents, pd.Series([other_percent], index=['Outros'])])
        wrapped_labels_pie_data = wrap_labels(pie_data.index)

        # Adicionando o gráfico de comparação total
        ax_total_comparison = fig.add_subplot(2, 2, 1)
        ax_total_comparison.pie(pie_data, labels=wrapped_labels_pie_data, autopct='%1.1f%%', startangle=90, labeldistance=1.3)
        ax_total_comparison.set_title('Total de Informações Sensíveis por Diretório', fontsize=10, fontweight='bold')

        # Gráficos de pizza para cada diretório principal com os dados mais recentes
        for i, directory in enumerate(top_directories.index, start=1):
            info = latest_data[latest_data['Diretório'] == directory]['Informação encontrada'].value_counts()
            wrapped_labels_info = wrap_labels(info.index)
            ax = fig.add_subplot(2, 3, i + 3)  # Isso vai posicionar os subplots na parte inferior da figura
            ax.pie(info, labels=wrapped_labels_info, autopct='%1.1f%%', startangle=90)
            ax.set_title(f'Top {i} diretório mais sensível', fontsize=10, fontweight='bold')
    ...
    def filtrado(self):
        ...
        tipos_arquivo = simpledialog.askstring("Tipos de arquivo",
                                               "Digite os tipos de arquivo que deseja filtrar (separados por vírgula) - Opções: CPF, Gênero, Religião, RG, Telefone: ")
        messagebox.showinfo("Scan em progresso",
